...
INTENT_TO_AGENTS = {
    "brainstorm":   ["brainstorm", "decomposition"],
    "research":     ["research", "systems_thinking"],
    "expand":       ["brainstorm", "research", "decomposition"],
    "dive_deeper":  ["brainstorm", "research", "systems_thinking", "critical_thinking"],
    "onboard":      ["onboarding"],
    "structure":    ["structure", "synthesis"],
    "summarize":    ["summary", "synthesis"],
    "critique":     ["critique", "critical_thinking"],
    "challenge":    ["socratic", "critical_thinking", "alternatives"],
    "decide":       ["decision_intelligence", "synthesis"],
    "reflect":      ["reflection", "synthesis"],
}


COG_STATE_TO_AGENTS = {
    "brainstorming": ["brainstorm", "decomposition", "visualization"],
    "stuck":         ["socratic", "alternatives", "critical_thinking"],
    "prioritizing":  ["decision_intelligence", "synthesis", "focus"],
    "reflecting":    ["reflection", "synthesis", "summary"],
    "deciding":      ["decision_intelligence", "critical_thinking", "synthesis"],
    "learning":      ["research", "decomposition", "brainstorm"],
    "research":      ["research", "systems_thinking", "decomposition"],
}

def classify_intent(state: AgentState) -> AgentState:
    """Determine which agents to invoke based on cognitive state and intent."""
    cog_state = state.get("cognitive_state", {}).get("state")
    intent = state.get("intent", "brainstorm")
    
    # Priority 1: Cognitive state mapping
    agents = COG_STATE_TO_AGENTS.get(cog_state, [])
    
    # Priority 2: Intent-based mapping (if cognitive state is neutral or unknown)
    if not agents:
        agents = INTENT_TO_AGENTS.get(intent, ["brainstorm"])
        
    # Always include core agents
    core_agents = ["memory", "learning", "visualization"]
    final_agents = list(set(agents + core_agents))
    
    return {**state, "agent_plan": final_agents, "error": None}
...
def merge_proposals(state: AgentState) -> AgentState:
    ...
    # Add thinking insights
    for insight in state.get("thinking_insights", []):
        proposals.append({
            "proposalType": "insight",
            "agentName": f"{insight['insight_type'].capitalize()}Agent",
            "payload": insight,
            "confidenceLevel": "high",
            "reasoning": f"Generated by {insight['insight_type']} analysis agent.",
            "sources": [],
        })
